...
<?php
    ...
    $id_user = $_SESSION['id'];
    $query = "SELECT * FROM user_elev WHERE id_user = '{$id_user}'";
    ...
    while($row = mysqli_fetch_array($select_profile_query)){
      ...
      if(isset($_POST['update_profile'])){
          ...
          $query = "UPDATE user_elev SET nume = '{$nume}',  prenume = '{$prenume}', ";
          $query .= "localitate = '{$localitate}', email = '{$email}',  scoala = '{$scoala}' WHERE id_user = '{$id_user}' ";
  ...
  if(isset($_POST['create_profile'])){
      ...
      $query = "INSERT INTO `user_elev`(`id_user`, `nume`, `prenume`, `localitate`, `scoala`, `email`) VALUES ('{$id_user}','{$nume}','{$prenume}','{$localitate}','{$scoala}','{$email}')";
      $update_query = mysqli_query($connection, $query);
      confirmQuery($update_query);
