<table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            <?php
                            $user_id = $_SESSION['id'];
                            $query = "SELECT * FROM recenzii LEFT JOIN users ON recenzii.id_user = users.user_id;";
                            ...
                            while($row = mysqli_fetch_assoc($select_recenzii)){
                                ...
                                $status  = $row['status'];
                                  ...
                                  echo "<td>{$data}</td>";
                                  echo "<td>{$status}</td>";
                                  echo "<td><a href='recenzie.php?aproba={$id_recenzie}'>Aproba</a></td>";
                                  echo "<td><a href='recenzie.php?respinge={$id_recenzie}'>Respinge</a></td>";
                            ...
                            ?>
                            <?php
                            if(isset($_GET['respinge'])){
                                $recenzie_id_s = $_GET['respinge'];
                                $query = "UPDATE recenzii SET status = 'respins' WHERE id_recenzie = {$recenzie_id_s} ";
                            ...
                            if(isset($_GET['aproba'])){
                                $recenzie_id_s = $_GET['aproba'];
                                $query = "UPDATE recenzii SET status = 'aprobat' WHERE id_recenzie = {$recenzie_id_s} ";
                                $result = mysqli_query($connection, $query);
                                header("Location: recenzie.php");
                            }
                            ?>
